...
def show():
    ...
    if x != "":
        ...
        url = 'http://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&APPID=d1a7ed688e1b9155f95106d9a55b9ab0&units=metric'.format(y,z)
...
entry_city=Entry(root,width=20)
entry_city.grid(row=0,column=1)
label_location=Label(root,text="Location",width=7,font=90,bg='pink')
label_location.grid(row=1,column=0,pady=6)
entry_lat=Entry(root,width=10)
entry_lat.grid(row=1,column=1)
entry_lon=Entry(root,width=10)
...
wind_direction.grid(row=15,column=0)
l10=Label(root,text="",bg='green')
l10.grid(row=15,column=1)
